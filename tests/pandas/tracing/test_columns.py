...
def test_eval_column_pattern():
    ...
    assert re.match(EVAL_COLUMN_PATTERN, "session_eval.name.label")
    assert re.match(EVAL_COLUMN_PATTERN, "session_eval.name.score")
    assert re.match(EVAL_COLUMN_PATTERN, "session_eval.name.explanation")
    assert re.match(EVAL_COLUMN_PATTERN, "trace_eval.name.label")
    assert re.match(EVAL_COLUMN_PATTERN, "trace_eval.name.score")
    assert re.match(EVAL_COLUMN_PATTERN, "trace_eval.name.explanation")
...
def test_eval_label_pattern():
    ...
    assert re.match(EVAL_LABEL_PATTERN, "session_eval.name.label")
    assert re.match(EVAL_LABEL_PATTERN, "trace_eval.name.label")
...
def test_eval_score_pattern():
    ...
    assert re.match(EVAL_SCORE_PATTERN, "session_eval.name.score")
    assert re.match(EVAL_SCORE_PATTERN, "trace_eval.name.score")
...
def test_eval_explanation_pattern():
    ...
    assert re.match(EVAL_EXPLANATION_PATTERN, "session_eval.name.explanation")
    assert re.match(EVAL_EXPLANATION_PATTERN, "trace_eval.name.explanation")
...
def test_eval_name_capture():
    matches = [
        ("eval.name_part.label", "eval.name_part"),
        ("eval.name with spaces.label", "eval.name with spaces"),
        ("session_eval.session_name.label", "session_eval.session_name"),
        ("trace_eval.trace_name.label", "trace_eval.trace_name"),
    ]
