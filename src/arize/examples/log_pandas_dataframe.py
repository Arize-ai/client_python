...
def main():
    ...
    embedding_features = getEmbeddingFeaturesDataFrame(
        NUM_RECORDS, EMBEDDING_SIZE
    )
    ...
    pred_labels["prediction_label"] = pred_labels["prediction_label"].astype(
        str
    )
    ids = pd.DataFrame(
        [str(uuid.uuid4()) for _ in range(NUM_RECORDS)],
        columns=["prediction_id"],
    )
    inferences = pd.concat(
        [features, embedding_features, pred_labels, ids], axis=1
    )
...
def getEmbeddingFeaturesDataFrame(
    n_records: int, emb_size: int
) -> pd.DataFrame:
    return pd.DataFrame(
        dict(
            ...
            token_array_data=[
                ["Token", "array", str(x)] for x in range(n_records)
            ],
        )
    )
