...
class CreateGeminiConfig(BaseModel):
    """
    ...
    def provider_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['GEMINI']):
            raise ValueError("must be one of enum values ('GEMINI')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in CreateGeminiConfig) in the input: " + _key)
