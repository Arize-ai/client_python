...
class MovingCustomBaselineWindow(BaseModel):
    """
    ...
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['MOVING']):
            raise ValueError("must be one of enum values ('MOVING')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in MovingCustomBaselineWindow) in the input: " + _key)

        _obj = cls.model_validate({
            "type": obj.get("type"),
            "moving_window_seconds": obj.get("moving_window_seconds"),
            "moving_window_delay_seconds": obj.get("moving_window_delay_seconds")
        })
        return _obj
