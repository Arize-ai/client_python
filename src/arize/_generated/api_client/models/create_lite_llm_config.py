...
class CreateLiteLlmConfig(BaseModel):
    """
    ...
    __properties: ClassVar[List[str]] = ["is_function_calling_enabled", "provider", "base_url", "api_key", "headers", "model_names"]

    @field_validator('provider')
    def provider_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['LITELLM']):
            raise ValueError("must be one of enum values ('LITELLM')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in CreateLiteLlmConfig) in the input: " + _key)
