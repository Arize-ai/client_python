...
class DynamicSingleThreshold(BaseModel):
    """
    ...
    __properties: ClassVar[List[str]] = ["type", "calculation", "operator", "multiplier"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['DYNAMIC_SINGLE']):
            raise ValueError("must be one of enum values ('DYNAMIC_SINGLE')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in DynamicSingleThreshold) in the input: " + _key)
