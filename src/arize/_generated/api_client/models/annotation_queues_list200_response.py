...
class AnnotationQueuesList200Response(BaseModel):
    """
    AnnotationQueuesList200Response
    """ # noqa: E501
    annotation_queues: List[AnnotationQueue] = Field(description="A list of annotation queues")
    pagination: PaginationMetadata
    __properties: ClassVar[List[str]] = ["annotation_queues", "pagination"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in annotation_queues (list)
        _items = []
        if self.annotation_queues:
            for _item_annotation_queues in self.annotation_queues:
                if _item_annotation_queues:
                    _items.append(_item_annotation_queues.to_dict())
            _dict['annotation_queues'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in AnnotationQueuesList200Response) in the input: " + _key)
