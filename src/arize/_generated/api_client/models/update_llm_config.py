...
class UpdateLlmConfig(BaseModel):
    ...
    model_names: Optional[List[StrictStr]] = Field(default=None, description="(`AWS_BEDROCK`, `CUSTOM`, `NVIDIA_NIM`, and `LITELLM` only) Replaces the custom model list. For `AWS_BEDROCK`, `CUSTOM`, and `NVIDIA_NIM` the effective config must keep at least one model source or the request is rejected with 422. Omit to keep unchanged.")
