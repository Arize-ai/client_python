...
class UpdateLlmConfig(BaseModel):
    ...
    base_url: Optional[StrictStr] = Field(default=None, description="(`CUSTOM`, `NVIDIA_NIM`, and `ANTHROPIC` only) New endpoint URL. For `NVIDIA_NIM` and `ANTHROPIC` the field is optional on the resource, so null clears it (falling back to the provider default endpoint). For `CUSTOM` it is required on the resource — null is rejected with 422. Omit to keep unchanged.")
