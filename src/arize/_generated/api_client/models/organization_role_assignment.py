...
class OrganizationRoleAssignment(BaseModel):
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if not isinstance(v, OrganizationCustomRoleAssignment):
            error_messages.append(f"Error! Input type `{type(v)}` is not `OrganizationCustomRoleAssignment`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into OrganizationCustomRoleAssignment
        try:
            instance.actual_instance = OrganizationCustomRoleAssignment.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
