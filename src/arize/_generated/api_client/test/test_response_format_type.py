# coding: utf-8

"""
    Arize REST API

    API specification for the backend data server. The API is hosted globally at https://api.arize.com/v2 or in your own environment. 

    The version of the OpenAPI document: 2.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from arize._generated.api_client.models.response_format_type import ResponseFormatType

class TestResponseFormatType(unittest.TestCase):
    """ResponseFormatType unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def testResponseFormatType(self):
        """Test ResponseFormatType"""
        # inst = ResponseFormatType()

if __name__ == '__main__':
    unittest.main()
