...
class MonitorsApi:
    ...
    def add_monitor_tags(
        ...
        Attach one or more existing tags to a monitor.  **Payload Requirements** - `tag_ids` is required and must contain between 1 and 100 tag IDs. - Every tag must already exist and belong to the same space as the   monitor. A tag from another space returns `422`. - Attaching a tag that is already attached is idempotent, so the same   request can be retried safely. - Unrecognized fields are rejected with `422` rather than ignored.  Returns `200` with the monitor's complete tag list, not `201`: attaching an existing tag creates no new resource.  **Valid example** ```json {   \"tag_ids\": [\"VGFnOjEyMzQ1\", \"VGFnOjEyMzQ2\"] } ```  **Invalid example** (empty list) ```json {   \"tag_ids\": [] } ``` ```json {   \"type\": \"https://arize.com/docs/ax/rest-reference/errors#validation-error\",   \"title\": \"Unprocessable Entity\",   \"status\": 422,   \"detail\": \"tag_ids must contain at least 1 tag ID\",   \"request_id\": \"req_01HZY6X8E7\" } ```  <Warning>This endpoint is in alpha, read more [here](https://arize.com/docs/ax/rest-reference#api-version-stages).</Warning> 
        ...
        _param = self._add_monitor_tags_serialize(
    ...
    @validate_call
    def add_monitor_tags_with_http_info(
        self,
        monitor_id: Annotated[StrictStr, Field(description="The unique monitor identifier (base64)")],
        add_tags_request: Annotated[AddTagsRequest, Field(description="Body containing the IDs of the tags to attach to the resource")],
        _request_timeout: Union[
    ...
    @validate_call
    def add_monitor_tags_without_preload_content(
        self,
        monitor_id: Annotated[StrictStr, Field(description="The unique monitor identifier (base64)")],
        add_tags_request: Annotated[AddTagsRequest, Field(description="Body containing the IDs of the tags to attach to the resource")],
        _request_timeout: Union[
    ...
    def _add_monitor_tags_serialize(
        self,
        monitor_id,
        add_tags_request,
        _request_auth,
    ...
    def list_monitor_tags(
        ...
        List the tags attached to a monitor.  Tags are shared within the space, so the same tag may appear on many resources. A monitor with no tags returns an empty list rather than a 404.  Requires read access to the monitor. A caller who cannot read it receives `404`, identical to the response for a monitor that does not exist.  <Warning>This endpoint is in alpha, read more [here](https://arize.com/docs/ax/rest-reference#api-version-stages).</Warning> 
        ...
        _param = self._list_monitor_tags_serialize(
            monitor_id=monitor_id,
            _request_auth=_request_auth,
            _content_type=_content_type,
            _headers=_headers,
    ...
    @validate_call
    def list_monitor_tags_with_http_info(
        self,
        monitor_id: Annotated[StrictStr, Field(description="The unique monitor identifier (base64)")],
        _request_timeout: Union[
            None,
    ...
    @validate_call
    def list_monitor_tags_without_preload_content(
        self,
        monitor_id: Annotated[StrictStr, Field(description="The unique monitor identifier (base64)")],
        _request_timeout: Union[
            None,
    ...
    def _list_monitor_tags_serialize(
        self,
        monitor_id,
        _request_auth,
        _content_type,
