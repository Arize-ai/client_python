# AnnotationQueueExampleRecordInput


## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**record_type** | **str** | Discriminator identifying the record source type. Must be &#x60;example&#x60; for dataset example records. | 
**dataset_id** | **str** | The dataset ID these examples belong to | 
**dataset_version_id** | **str** | Optional. The specific dataset version to use. If omitted, the latest version is used.  | [optional] 
**example_ids** | **List[str]** | Optional. List of example IDs within the dataset to add to the queue. If omitted, all examples in the dataset (or dataset version) are added.  | [optional] 

## Example

```python
from arize._generated.api_client.models.annotation_queue_example_record_input import AnnotationQueueExampleRecordInput

# TODO update the JSON string below
json = "{}"
# create an instance of AnnotationQueueExampleRecordInput from a JSON string
annotation_queue_example_record_input_instance = AnnotationQueueExampleRecordInput.from_json(json)
# print the JSON string representation of the object
print(AnnotationQueueExampleRecordInput.to_json())

# convert the object into a dict
annotation_queue_example_record_input_dict = annotation_queue_example_record_input_instance.to_dict()
# create an instance of AnnotationQueueExampleRecordInput from a dict
annotation_queue_example_record_input_from_dict = AnnotationQueueExampleRecordInput.from_dict(annotation_queue_example_record_input_dict)
```
[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


