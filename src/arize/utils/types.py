    ...
    def _validate_embedding_vector(
        ...
        if not Embedding._is_valid_iterable(self.vector):
            raise TypeError(
                ...
                f"{type(self.vector)}. Must be list, np.ndarray or pd.Series"
            )
...
class ObjectDetectionLabel(NamedTuple):
    ...
    scores: Optional[List[float]] = (
        None  # Actual Object Detection Labels won't have scores
    )
...
class Schema(BaseSchema):
    ...
    embedding_feature_column_names: Optional[
    ...
    object_detection_prediction_column_names: Optional[
        ObjectDetectionColumnNames
    ] = None
    object_detection_actual_column_names: Optional[
        ObjectDetectionColumnNames
    ] = None
...
def add_to_column_count_dictionary(
    column_dictionary: Dict[str, int], col: Optional[str]
):
    if col:
