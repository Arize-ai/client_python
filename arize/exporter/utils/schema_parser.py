...
def get_object_detection_actual(cols: Iterable[str]) -> Optional[ObjectDetectionColumnNames]:
...
def get_arize_schema(df: pd.DataFrame) -> Schema:
    ...
    prediction_score_column_name = get_pred_score(cols) or get_numeric_pred(cols)
    actual_score_column_name = get_actual_score(cols) or get_numeric_actual(cols)
    ...
    schema = Schema(
        ...
        object_detection_prediction_column_names=get_object_detection_prediction(cols),
