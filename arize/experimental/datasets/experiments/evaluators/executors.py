...
from arize.experimental.datasets.experiments.evaluators.exceptions import ArizeException
    ...
    async def consumer(
        ...
        while True:
            ...
            try:
                ...
                if (retry_count := abs(priority)) < self.max_retries and not is_arize_exception:
                    tqdm.write(
                ...
                try:
                    for attempt in range(self.max_retries + 1):
                        ...
                        try:
                            ...
                            execution_details[index].log_exception(exc)
                            is_arize_exception = isinstance(exc, ArizeException)
                            if attempt >= self.max_retries or is_arize_exception:
                                raise exc
                            else:
