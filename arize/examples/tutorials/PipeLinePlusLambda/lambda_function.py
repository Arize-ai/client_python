...
def lambda_handler(event, context):
    ...
    try:
        ...
        content_object = s3.Object(default_bucket,
                                   s3_schema_key_prefix + '/schema.txt')
        if verbose: print('content_object')
        if not content_object:
            print('content_object not found')
            print(f'default bucket {default_bucket}')
            print(f'{s3_schema_key_prefix}/schema.txt not found')
        if verbose: print('file_content')
        file_content = content_object.get()['Body'].read().decode('utf-8')
        if verbose: print('json_content')
        json_content = json.loads(file_content)
        query_list = []
        columns = []
        if verbose: print(json_content['input'])
        if verbose: print('event')
        if verbose: print(event)
        if verbose: print('data send')
        if verbose: print(data_send)
        if verbose: print('data_send data')
        if verbose: print(type(data_send))
        if verbose: print(json.dumps(data_send))
        data_val = json.loads(data_send)
        if verbose: print('data_val')
        if verbose: print(data_val)
        ...
        if verbose: print('byte array')
        data_send_byte_array = bytearray(json.dumps(payload), encoding='utf8')
        if verbose: print('calling runtime')
        if verbose: print(data_send_byte_array)
    ...
    if verbose: print(response_data)
    ## ARIZE CLIENT SETUP ##
    ...
    tfuture = arize_client.log_prediction(model_id=model_name,
                                          model_version=model_version_id_now,
                                          features=features,
                                          prediction_id=prediction_id,
                                          prediction_label=response_data)
    if verbose: print(tfuture.result())
    if verbose: print([features, response_data, prediction_id])
    if verbose: print('result is!!!!')
    if verbose: print(response_data)
