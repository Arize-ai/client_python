...
def lambda_handler(event, context):
    ...
    if event['detail']['TransformJobStatus'] == 'Completed':
        ...
        if 'ArizeMonitor' in event['detail']['Environment'] and event[
                'detail']['Environment']['ArizeMonitor'] == "1":
            ...
            if 'Environment' in event['detail']:
                ...
                output_path = event['detail']['TransformOutput'][
                    'S3OutputPath']
                ...
                column_names = list(
                    features['features'].values) + ['predictions']
                output_df = pd.read_csv(io.StringIO(output),
                                        sep=",",
                                        names=column_names)
                predictions_df = output_df['predictions']
                features_df = output_df.drop(['predictions'], axis=1)
                ## ARIZE CLIENT SETUP ##
                arize_client = Client(organization_key=org_key,
                                      api_key=api_key)
